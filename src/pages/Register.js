...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    let data = {
      sur_name:
        formdata.sur_name[0].toUpperCase() +
        formdata.sur_name.slice(1).toLowerCase(),
      first_name:
    ...
    try {
      const response = await fetch(`${REACT_APP_BACKEND_URL}/register_1/`, {
        method: "POST",
        credentials: "include",
        headers: {
  ...
  const validateForm = () => {
    ...
    const reg_regex =
      /^(0?[1-9]?[0-9]|100)[\/\-_]([a-zA-Z])[\/\-_]([a-zA-Z]+)[\/\-_](\d{2,})[\/\-_]([a-zA-Z]+)$/;
