    ...
    const getData = async () => {
      try {
        const response = await fetch(
          `${REACT_APP_BACKEND_URL}/member/relation/${id}`,
          {
            method: "GET",
            credentials: "include",
            headers: {
    ...
    const updateData = async () => {
      try {
        for (const item of data) {
          const response = await fetch(
            `https://127.0.0.1:8000/member/relation/${id}`,
            {
              method: "POST",
              credentials: "include",
              headers: {
    ...
    <>
      ...
      <div className="profile-item">
        {(rights.member_id == id || rights.perm.info_3 >= 2) && (
          <div className="edit-icon" onClick={() => setRegister(true)}>
            <LuPenSquare />
          </div>
        )}
          ...
          <div>
            {relation.length > 0 ? (
              relation.map((item, index) => (
                <p
                  key={index}
                >{`${item.relation} : ${item.name}: ${item.contact}`}</p>
              ))
            ) : (
              <p>No data found</p>
            )}
      ...
      <div className="profile-item">
        {(rights.member_id == id || rights.perm.info_3 >= 2) && (
          <div className="edit-icon" onClick={() => setRegisterFriend(true)}>
            <LuPenSquare />
          </div>
        )}
        <div className="details">
